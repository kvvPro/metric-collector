...
func InitDelay(delay time.Duration) Option {
	return func(c *Config) {
		c.initDelay = delay
	}
}

func Step(step time.Duration) Option {
	return func(c *Config) {
		c.stepDelay = step
	}
}
...
// kvv
// StepDelay is a DelayType which increase delay by step on after each attempt
func StepDelay(n uint, _ error, config *Config) time.Duration {
	config.delay = time.Duration(uint64(n)*uint64(config.stepDelay)) + config.initDelay
	return config.delay
}
